...
export class Driver {
  ...
  atomicWithLock(key, transform, options = {}, fn = _.noop) {
    if (options instanceof Function) {
      fn = options;
      options = {};
    }
    ...
    const ropts = {
      time: opts.atomicRetryTimes,
      interval: options.atomicRetryInterval
    };

    debug(`Driver.atomicWithLock. key: ${key} %j`, ropts);
  ...
  atomicNoLock(key, transform, options = {}, fn = _.noop) {
    if (options instanceof Function) {
      fn = options;
      options = {};
    }
    ...
    debug(`Driver.atomicNoLock. key: ${key} %j`, ropts);
    async.retry(ropts, rfn => {
